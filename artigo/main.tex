...
continuidade durante desconexões --- e então submete essas garantias a
\textbf{injeção de falhas sistemática}, não apenas a testes unitários
...
respectivamente). A correção não elimina toda a resiliência residual: um
terceiro experimento mostra que, se o próprio processo herdeiro morrer
antes do processo que ele protege, \textbf{\statExpRFiveHeirDeadPct{}\% do
conteúdo é perdido} --- um gap real, confirmado mas ainda não corrigido
nesta versão do trabalho. Uma validação adicional em rede real (nós BEAM
...
é um artefato. Complementarmente, o sistema é validado com treino
federado \textbf{real} --- um MLP sobre partições non-IID do MNIST via
\textit{Axon}, não apenas tensores sintéticos --- confirmando speedup de
...
desconexão via ring buffer, e uma comparação empírica direta (não apenas
qualitativa) com SSP~\cite{ho2013} e FedAsync~\cite{xie2019} que revela um
resultado misto e é reportada como tal: o AFL supera uma barreira de
staleness limitada inspirada em SSP em throughput sob \textit{stragglers}
extremos, mas perde para o FedAsync bem-ajustado em velocidade de
convergência ($p<10^{-4}$) --- vantagem que desaparece sob um
hiperparâmetro mal-ajustado --- devido a um passo de mistura que decai
...
\begin{enumerate}[leftmargin=*,noitemsep]
  ...
  \item \textbf{Dois gaps de resiliência reais, descobertos e corrigidos}: o
        ...
        próprio herdeiro (um terceiro gap real, confirmado mas
  ...
  \item \textbf{Validação com treino real}: os mecanismos acima são
        ...
        non-IID do MNIST --- não apenas sobre tensores sintéticos ---
        permitindo medir impacto real em acurácia e loss de validação, e
        uma comparação empírica direta (não só qualitativa) com SSP e
        FedAsync.
\end{enumerate}
...
vez de um peso de mistura fixo --- Seção~\ref{sec:results} (Experimento~G)
compara os dois empiricamente, não apenas qualitativamente como em uma
...
harness --- o resultado é misto, não uma vitória geral do AFL (discutido em
...
Diferentemente de uma versão preliminar deste trabalho, que media
...
não o comportamento de aprendizado ---, a convergência é medida aqui sobre um
...
em H1; $|r|>0{,}5$ em todos os tamanhos de buffer testados em H2). A
exceção é o Experimento B (H3): a única fonte de variância entre repetições
--- qual dos 4 nós remanescentes avança o modelo em cada rodada da
...
uma regra a priori de tamanho de efeito mínimo detectável, uma limitação
...
o que mede apenas a álgebra da Eq.~\ref{eq:effective_nk}, não o comportamento
...
\begin{enumerate}[noitemsep,leftmargin=*]
  \item \textbf{Modelo fresco:} pré-treina-se o MLP global por 15 rodadas de
    FedAvg real sobre os 5 nós non-IID --- ponto de referência $(\text{W}_{\text{fresh}},
  ...
  \item \textbf{Update atrasado:} um nó $k$ (rotacionado a cada repetição)
    ...
    --- produz $W_k$ independente do gap;
...
conjunto de validação \textit{held-out} (800 amostras) após cada rodada ---
10 repetições independentes com IC 95\% via distribuição $t$ de Student.
...
mais lento --- uma barreira de staleness, não a ausência total de barreira
do AFL. A barreira é verificada por \textit{polling} a cada 1\,ms (overhead
pequeno frente às latências simuladas de 10--500\,ms, mas não nulo --- uma
implementação orientada a eventos não o teria). Testa-se
...
staleness e throughput --- não apenas dois pontos extremos.
...
para o braço AFL e para o braço FedAsync --- o que significa que parte de
...
vez e reutilizados por \emph{todos} os braços (AFL e cada $\alpha_0$) ---
como \texttt{AFL.Model.local\_train/4} é determinístico dado
...
de mistura --- não o agendamento nem o mecanismo de tratamento de
...
(EdgeNode) imediatamente depois --- medindo se o conteúdo bufferizado
...
intervalo entre o crash do herdeiro e o próximo crash do dono --- que pode
ser arbitrariamente longo, não apenas um instante de corrida. Este
...
sintéticos $d=10$ (Seção~\ref{sec:exp}) --- mede especificamente o efeito de
...
para $B{=}0$ --- correção de Holm-Bonferroni sobre a família de
...
\statHolmFamilySize{} testes, Seção~\ref{sec:stat_methodology}) --- um resultado real e estatisticamente significativo, não o
...
explícita, logo diferentes a cada execução --- eliminou por completo a
...
\textbf{13,6 rodadas} (mín. 4, máx. 26 entre as 10 repetições --- variância
...
variância visível entre repetições --- evidência direta de que o mecanismo de
FedAvg incremental (Eq.~\ref{eq:fedavg_inc}) converge sob aprendizado de
máquina genuíno e não apenas sob a álgebra de médias móveis de tensores
sintéticos.
...
todos os braços --- e aqui o resultado \emph{não} favorece o AFL, para
...
e com desvio-padrão $2{,}7\times$ maior que os demais braços. Isso revela
...
ótima para convergência sob um número finito de rodadas assíncronas ---
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \textbf{Residual de resiliência --- confirmado empiricamente, não
    ...
    --- porque o \texttt{:heir} de uma tabela ETS é vinculado ao PID
    ...
    \texttt{:heir} via \texttt{:ets.setopts/2} --- não implementada nesta
...
verificação empírica sistemática em vez de aceitá-la por inspeção de
código. O resultado central é que essa suposição estava
...
via injeção de falhas sistemática, e dois gaps reais --- um dos quais só
veio à luz numa segunda rodada de revisão adversarial, após o primeiro já
parecer resolvido --- que essa metodologia expôs e permitiu corrigir com
evidência antes/depois, não apenas por argumento de design.
...
\textbf{real} (MLP via \textit{Axon} sobre MNIST non-IID, não apenas
...
\begin{itemize}[noitemsep,leftmargin=*]
  ...
  \item \textbf{H3}: A penalidade hiperbólica reduz em \textbf{\statExpBHypReductionGapTwenty\%} a
    ...
    descartar nenhuma
    atualização --- não por ``eliminar bias'' de um cenário adversarial
    sintético, mas por conter a variância de um efeito que, sob dados
...
\textbf{misto} --- e é reportada como tal, não filtrada para favorecer o
...
mesmo protocolo non-IID --- vantagem que desaparece para $\alpha_0=0{,}9$
...
confirmado --- não apenas teórico --- pelo Experimento~R5
...
dinamicamente e caracterizar falhas em cascata entre vários EdgeNodes ---
o Experimento~R4 (Seção~\ref{sec:r4}) já descarta interação problemática
