...
\textbf{injeção de falhas sistemática} (\textit{chaos engineering}), em
vez de testes unitários anedóticos. O resultado central é que a
...
\textbf{100\% do conteúdo é perdido}; encadear um segundo
...
quantitativa antes/depois, em vez de argumento de design, que expôs
...
literatura de FL, é a \textbf{verificação} da tolerância a falhas, e não
apenas sua suposição. Trabalhos que implementam FL sobre runtimes com
...
processo, verificar que ele volta), e não pela prática de \textit{Chaos
...
próprio \textit{crash} do processo que as criou. Trata-se de um gap real,
...
mecânica de agregação em si não é o objeto de estudo deste trabalho, e
...
\texttt{:heir} mais supervisão, aplicada a ambos os componentes):
...
monitorado propagar; é determinístico, sem corrida com o restart
...
processo falhando por vez, o que deixa em aberto se
...
($\ell = [10,10,20,50,500]$\,ms). Aqui, diferentemente do
...
os nós, e não só os gradientes de um nó. Sob o mecanismo
...
injetado. A verificação foi direta, além da simples contagem de versão:
...
depois causava perda de 100\% do conteúdo bufferizado, e não só em
...
e não só na prática. Para o \texttt{AFL.ModelKeeper} especificamente,
...
falhas cobrindo cada processo do sistema individualmente, e não só os
...
\begin{enumerate}[noitemsep,leftmargin=*]
  \item \textbf{Escala do modelo:} os Experimentos B/D/E usam um MLP de
    ...
    real, e não só especulativo: para o MLP de \statScaleLargeParams{}
    ...
    gargalo real de memória nessa escala, e não só hipotético, ainda que
...
sobre plataformas com tolerância a falhas nativa, a de que a resiliência é
...
permitiu corrigir com evidência antes/depois, em vez de argumento de
design.
...
falha concorrente (R4, Seção~\ref{sec:r4}), com EdgeNode e Aggregator
...
o herdeiro reinicia; encadear um segundo herdeiro não fecharia essa
...
camada de abstração (processo/ETS para persistência em disco), em vez de
simplesmente adicionar mais um processo; e essa diferença só é visível por verificação
...
regiões de nuvem), diferente da bridge local do Docker usada nesta
versão;
